...
public class Shell {
    ...
    private static void shellCp(final String[] args) throws Exception {
        ...
        if ((args[args.length - 1].isEmpty()) || (args[args.length - 2].isEmpty())) {
            shellNoName("cp");
        }
        if (args.length == 3) {
            try {
            ...
            try {
                ...
                if (Paths.get(pathToNewFile.toString(), pathToFile.getFileName().toString()).normalize().toString().equals(pathToFile.toString())) {
                    shellEqualNames("cp");
                }
                ...
                if (names.length != 0) {
                    for (String name : names) {
                        if (Paths.get(pathToFile.toString(), name).normalize().toFile().isDirectory()) {
                            System.setProperty("user.dir", pathToFile.toString());
                            String[] helpArray = new String[] {"cp", "-r", name, pathToNewFile.toString()};
                            shellCp(helpArray);
                            System.setProperty("user.dir", pathToFile.getParent().toString());
                        }
                        if (Paths.get(pathToFile.toString(), name).normalize().toFile().isFile()) {
                            String[] helpArray = new String[] {"cp", name, pathToNewFile.toString()};
                            shellCp(helpArray);
                        }
                    }
                }
    ...
    private static void shellAccessProhibited(final String commandName, final String name) throws Exception {
        throw new Exception(commandName + ":  access to list of [" + name + "] is phohibited.");
    }
    
    private static void shellAlreadyExist(final String commandName, final String name) throws Exception {
    ...
    private static void shellInvalidName(final String commandName, final String name) throws Exception {
        throw new Exception(commandName + ": [" + name + "] is invalid name.");
    }

    private static void shellNotDirectory(final String commandName, final String name) throws Exception {
    ...
    private static void shellNotExist(final String commandName, final String name) throws Exception {
        throw new Exception(commandName + ": the file [" + name + "] is not exists.");
    }
    
    private static void shellNotMkdir(final String commandName, final String name) throws Exception {
    ...
    private static void shellSecurity(final String commandName, final String name) throws Exception {
    ...
    private static void shellWrongKey(final String commandName, final String key) throws Exception {
